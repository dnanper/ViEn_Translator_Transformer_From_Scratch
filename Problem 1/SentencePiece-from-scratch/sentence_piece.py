...
class SentencePieceTrainer:
    ...
    def generalized_backward_step(self, text, p):
        ...
        while idx > 1:
            if not p[idx-1]:
                # No valid path - fallback to character-level for this position
                # Try to look back and find the nearest valid token
                found = False
                for lookback in range(1, min(idx, self.maxlen + 1)):
                    test_token = text[idx-lookback-1:idx-1]
                    if self.trie.get_value(test_token):
                        # Found a valid token
                        tokenization.append(test_token)
                        idx -= lookback
                        found = True
                        break
                
                if not found:
                    # Still no valid token - use single character
                    tok = text[idx-2:idx-1]
                    tokenization.append(tok)
                    idx -= 1
            else:
